...
--SUBSTR(EMAIL, 1, INSTR(EMAIL,'@')->SUN_DI@
...
FROM DUAL;  --123, 기본값 1의 자리까지 표기 
            --소수점 0번째자리까지 표기 = ROUND(123.456,0)

SELECT ROUND(123.456,1) 
FROM DUAL;  --123.5, 소수점 첫번째자리까지 표기

SELECT ROUND(123.456,-1) 
FROM DUAL;  --120, 10의 자리까지 표기

SELECT ROUND(123.456,-2) 
FROM DUAL;  --100, 100의 자리까지 표기
...
FROM DUAL; --123 소수점 아래 버림

SELECT TRUNC(123.456,1)
FROM DUAL;  --123.4 소수점 첫째자리 아래 버림

SELECT TRUNC(123.456,-1)
FROM DUAL;  --120 10의 자리 아래 버림
...
-- SYSTIMESTAMP : SYSDATE + MS 단위 추가 (TIMESTAMP : 특정 시간을 나타내거나 기록하기 위한 문자열)
SELECT SYSTIMESTAMP 
FROM DUAL;
...
--EMPOLYEE 테이블에서 사원의 이름, 입사일, 근무한 개월 수, 근무 년차 조회
SELECT EMP_NAME,HIRE_DATE,CEIL (MONTHS_BETWEEN(SYSDATE, HIRE_DATE))"근무한 개월 수",
CEIL (MONTHS_BETWEEN(SYSDATE, HIRE_DATE)/12)||'년차'"근무 년차"
FROM EMPLOYEE;
--|| : 연결 연산자(문자열 이어쓰기)

-- ADD_MONTHS(날짜 , 숫자) : 날짜에 숫자만큼의 개월 수를 더함.
SELECT ADD_MONTHS() MONTH(SYSDATE, 4) 
FROM DUAL; 

SELECT ADD_MONTHS(SYSDATE, -1) 
FROM DUAL; 

-- LAST_DAY(날짜) : 해당 달의 마지막 날짜를 구함. (윤년 판별, 정기배송 상품, 이자)
...
SELECT SYSDATE 
FROM DUAL; --2022-09-01 11:21:47.000

--2022/09/01 11:21:47 목요일
...
FROM DUAL;  --년,월,일이 날짜를 나타내는 패턴으로 인식이 안되서 오류 발생-> " "로 문자로 인식시킴
...
SELECT TO_DATE('2022-09-02')
FROM DUAL;

SELECT TO_DATE(20220902)
FROM DUAL;

SELECT TO_DATE('220901 113255', 'YYMMDD HH24MISS')"시간" 
FROM DUAL; --패턴을 적용해 작성된 문자열의 각 문자가 어떤 날짜인지 인지시킴
...
SELECT COUNT(*)
FROM EMPLOYEE; --EMPLOYEE 테이블의 행의 개수
...
FROM EMPLOYEE;  --6
--NULL값을 제외하고 실제 값이 있는 행의 개수만 조회
