/*  
   ...
   : SQL 명령문 중 SELECT의 처리 속도를 향상 시키기 위해 컬럼에 대해서 생성하는 객체
	 인덱스 내부 구조는 B* 트리 형식(향상된 이진트리)으로 되어있음.
   ...
   인덱스를 이용한 검색 방법
   WHERE절에 INDEX가 지정된 컬럼을 언급
   
*/

SELECT *FROM EMPLOYEE 
WHERE EMP_ID=215;       --INDEX사용 O 

SELECT *FROM EMPLOYEE 
WHERE EMP_NAME='대북혼';  --INDEX사용 X
...
WHERE TEST_ID='TEST500000'; --50ms

SELECT*FROM TB_IDX_TEST 
WHERE TEST_NO=500000;       --2ms
