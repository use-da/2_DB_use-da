/* 
	...
	-논리적 가상 테이블 -> 테이블 모양을 하고 있지만 실제로 값을 저장하고 있지 않음  
	...
	***사용 시 주의사항***
	1) 가상의 테이블(실제 테이블 X) -> ALTER구문 사용불가
    ...
    **4) WITH READ ONLY 옵션 : 뷰에 대해 조회만 가능(DML 수행 불가)  
    5) [(alias[,alias]...)] : 컬럼 별칭 (자주 사용하지 않음)

 */

-- 사번, 이름, 직급명, 조회 결과를 저장하는 VIEW 생성
...
-- OR REPLACE 추가 후 실행 -> 기존 V_EMP를 새로운 VIEW로 덮어쓰기
...
-- ORA-01400: NULL을 ("LYH"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다

--> VIEW를 이용한 INSERT 시 원본 테이블에 삽입이 진행
...
--  VIEW의 본래 목적인 조회에만 사용하는게 좋다
--> WITH READ ONLY로 VIEW의 오용을 막을 수 있다
