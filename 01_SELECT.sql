...
--DUAL(DUmmy tAbLe) : 더미 테이블, 임시 조회용 테이블
...
--주의사항 1) DISTINCT구문은 SELECT마다 한 번 씩만 작성 가능
--주의사항 2) DISTINCT구문은 SELECT 제일 앞에 작성되어야한다
...
--EMPLOYEE 테이블에서 급여가 300 초과인 사원의 사번,이름,급여,부서코드를 조회
...
---------------------------------------------------
--논리연산자 (AND,OR)
--EMPLOYEE테이블에서 급여가 300만 미만 또는 500만 이상인 사원의 사번, 이름, 급여, 전화번호 조회

SELECT EMP_ID,EMP_NAME,SALARY,PHONE
FROM EMPLOYEE
WHERE SALARY <3000000 OR SALARY >= 5000000;

--EMPLOYEE테이블에서 급여가 300만 이상 500만 미만인 사원의 사번, 이름, 급여, 전화번호 조회
...
--NOT BETWEEN AND B : A이상 B이하가 아닌 경우-> A미만 B초과
...
--오라클은 데이터 타입이 달라고 형태가 일치하면 자동으로 타입을 변환시킴
...
-->문제점 : _를 기준점으로 삼았으나 와일드 카드'_'와 동일한 표기법으로 작성되어 구분되지 않음
--해결 방법 : LIKE의 ESCAPE OPTION을 이용하여 _를 구분한다

--LIKE의 ESCAPE OPTION : 일반 문자로 처리할 '_' , '%' 앞에 아무 특수기호를 첨부해 구분한다
SELECT EMP_NAME,EMAIL
FROM EMPLOYEE 
...
--연습문제
--EMPLOYEE 테이블에서 이메일'_'앞이 4글자이면서 부서코드가 'D9' 또는 'D6'이고 
--입사일이 1990-01-01~2000-12-31이고 급여가 270만 이상인 사원의 
--사번,이름,이메일,부서코드,입사일,급여 조회
--**AND가 OR보다 우선순위가 높다, ()사용가능하다
...
--EMPLOYEE 테이블에서 부서코드가 D1,D6,D9가 아닌 사원의 사번, 이름, 부서코드 조회
SELECT EMP_ID,EMP_NAME,DEPT_CODE
FROM EMPLOYEE 
WHERE DEPT_CODE NOT IN ('D1','D6','D9') --12명
OR DEPT_CODE IS NULL; --부서코드 없는 2명 (NULL처리 연산자)
...
--EMPLOYEE테이블에서 보너스가 없는 사원의 이름, 보너스 조회
SELECT EMP_NAME,BONUS
FROM EMPLOYEE 
WHERE BONUS IS NULL;
...
--EMPLOYEE테이블 급여 오름차순으로
--사번,이름,급여 조회
SELECT EMP_ID,EMP_NAME,SALARY
FROM EMPLOYEE
ORDER BY SALARY; --ASC가 기본값 오름차순(점점 오름)

--급여 200만 이상인 사원의 사번, 이름, 급여 조회 단, 급여 내림차순으로 조회
SELECT EMP_ID,EMP_NAME,SALARY
FROM EMPLOYEE 
