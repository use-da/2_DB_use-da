...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID); --연결고리역할 컬럼
...
--LOCATION 테이블               DEPARTMENT   테이블   
--LOCAL_CODE    지역코드        DEPT_ID      부서코드
--LOCAL_NAME    지역명          DEPT_TITLE   부서명
--NATIONAL_CODE 국가코드        LOCATION_ID  지역코드
...
--INNER JOIN시 문제점
--연결에 사용된 컬럼 값이 일치하지 않으면 조회 결과에 포함되지 않는다
...
-->왼 편에 작성된 테이블의 모든 행이 결과에 포함되어야한다.(JOIN이 안되는 행도 결과에 포함)
...
-->JOIN 구문을 잘못 작성하는 경우 CROSS JOIN의 결과가 조회됨(오류확인용)
...
--4. 보너스포인트를 받는 직원들의 사원명, 보너스포인트, 부서명, 근무지역 명을 조회하시오.
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME,JOB_NAME,SALARY 급여,12*SALARY(1+NVL(BONUS,0)) "연봉"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID)
JOIN LOCATION ON(LOCATION_ID=LOCAL_CODE)
JOIN NATIONAL USING(NATIOAL_CODE)
