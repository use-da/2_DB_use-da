...
FROM EMPLOYEE
GROUP BY DEPT_CODE-->DEPT_CODE가 같은 행끼리 하나의 그룹
--ORA-00937: 단일 그룹의 그룹 함수가 아닙니다

--EMPLOYEE 테이블에서 직급 코드가 같은 사람의 직급코드, 급여 평균, 인원 수를 직급코드 오름차순으로 조회
...
SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여')성별,
	...
	SUM(SALARY)"급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여')--별칭사용 X(SELECT절 해석X)
...
-- *** GROUP BY 사용시 주의사항
--> SELECT문에 GROUP BY절을 사용할 경우 조회하려는 컬럼 중
...
FROM EMPLOYEE
GROUP BY DEPT_CODE,JOB_CODE --DEPT_CODE로 그룹을 나누고 나눠진 그룹 내에서 JOB_CODE로 또 분류  
...
FROM EMPLOYEE
--WHERE SALARY >=3000000  한 사람의 급여가 300이상
GROUP BY DEPT_CODE
HAVING AVG(SALARY)>=3000000 --DEPT_CODE 중 급여 평균이 300이상인 그룹
ORDER BY DEPT_CODE;

--EMPLOYEE 테이블에서 직급별 인원 수가 5명 이하인 직급코드, 인원수 조회(직급코드 오름차순)
