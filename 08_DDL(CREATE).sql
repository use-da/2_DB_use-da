...
-- TCL(Transaction Control Language) : 트랜잭션 제어 언어, DML수행 내용을 COMMIT, ROLLBACK하는 언어
...
/* 자료형
    NUMBER : 숫자형(정수, 실수), (,)까지 자동 반올림처리
			 오류가 종종 있어 NUMBER 그대로 사용((,)없이)    
    ...
    1BYTE*(2^10)=1KB   1KB*(2^10)=1MB  1MB*(2^10)=1GB
    데이터베이스에 파일저장은 지양
*/
...
--SQL작성법 : 대문자 작성 권장, 연결된 단어 사이에 "_" 사용
--문자인코딩 UTF-8 : 영어, 숫자 1BYTE
--				  한글, 특수문자 등은 3BYTE취급
...
--INSERT나 UPDATE시 컬럼값으로 DEFAULT를 작성하면 테이블 생성 시 해당 컬럼에 지정된 DEFAULT 값으로 삽입
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO MEMBER VALUES('MEM02','123ABC','김영희','971213-2345678',SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO MEMBER VALUES('MEM03','123ABC','박철수','941213-1234568',DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO MEMBER(MEMBER_ID,MEMBER_PWD,MEMBER_NAME) VALUES('MEM04','123ABC','오민수');

--*JDBC에서 날짜를 입력 받았을 때 삽입하는 방법 '2022-09-13 17:33:27'
INSERT INTO MEMBER VALUES('MEM05','123ABC','이지연','931213-2345679',
...
CREATE TABLE MEMBER2(
	MEMBER_ID VARCHAR2(20),
	MEMBER_PWD VARCHAR2(20),
	MEMBER_NMAE VARCHAR2(30),
	MEMBER_TEL NUMBER
);

INSERT INTO MEMBER2 VALUES('MEM01','PASS01','고길동',7712341234);
INSERT INTO MEMBER2 VALUES('MEM02','PASS02','고길순',01012341234);
-->NUMBER 타입 컬럼에 데이터 삽입 시 제일 앞에 0이 있으면 이를 자동으로 제거함
-->전화번호, 주민등록번호처럼 숫자로만 되어있는 데이터지만 0으로 시작할 가능성이 있으면 CHAR, VARCHAR2 같은 문자형을 사용
...
-- ORA-01400: NULL을 ("LYH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
--------------------------------------------------------------------------------------------------------------------
...
--> ORA-00001: 무결성 제약 조건(LYH.USER_ID_U)에 위배됩니다
...
--USER_ID, USER_NAME 묶어서 제약조건 설정
--(USER01, 홍길동)(USER02, 홍길동) 중복X
--(USER01, 김길동)(USER02, 홍길동) 중복X
...
-->기본키 중복으로 오류
-->ORA-00001: 무결성 제약 조건(LYH.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
-->기본키가 NULL 이므로 오류
-->ORA-01400: NULL을 ("LYH"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
--ORA-00001: 무결성 제약 조건(LYH.PK_USERNO_USERID)에 위배됩니다
...
--ORA-01400: NULL을 ("LYH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
-- 제공되는 값 외에 NULL(참조하는 값이 없음)을 사용할 수 있음
...
-->값50은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로 외래키 제약 조건에 위배되어 오류 발생.
-->ORA-02291: 무결성 제약조건(LYH.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
-->ORA-02292: 무결성 제약조건(LYH.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
--ORA-02290: 체크 제약조건(LYH.GENDER_CHECK)이 위배되었습니다

-- CHECK 제약 조건은 범위로도 설정 가능.
--BETWEEN, OR....

-- *CHECK 제약 조건은 FK 제약조건의 하위호환 
--> 컬럼 값을 지정된 형태로만 제한을 하는데 CHECK는 작성 후 수정이 번거롭지만
...
CREATE TABLE USER_TEST(
	...
	GENDER VARCHAR2(20) CONSTRAINT CK_GENDER CHECK(GENDER IN('남','여')),
	PHONE VARCHAR2(20),
...
-->서브쿼리로 테이블 생성 시 테이블의 형태(컬럼명, 데이터 타입)+ 제약조건NOT NULL만 복사되고
-- 제약조건, 코멘트는 복사되지않아 추가작업 필요
