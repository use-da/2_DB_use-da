...
-- COMMIT(트랜잭션 종료 후 저장)TRANSACTION(임시저장소) 내용 DB에 반영
-- ROLLBACK(트랜잭션 취소)TRANSACTION(임시저장소)에 있는 내용 제거 
-- SAVEPOINT(임시저장) ROLLBACK시 제거X
...
--INSERT확인
SELECT*FROM DEPARTMENT2 ;
--DB에 반영 된 것처럼 보이지만 SQL 수행 시 트랜잭션 내용도 포함해서 수행(실제 DB반영X)
