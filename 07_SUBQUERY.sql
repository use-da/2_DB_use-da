...
/*  서브쿼리 유형
    ...
    - 단일행(단일열=컬럼1) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 1개일 때 
    
    - 다중행(단일열=컬럼1) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 여러개일 때
...
-- 전 직원의 급여 평균보다 많은 급여를 받는 직원의 이름, 직급, 부서, 급여를 직급 순으로 정렬하여 조회
...
ORDER BY JOB_CODE;
--*SELECT절에 없는 컬럼이라도 FROM, JOIN으로 존재하는 컬럼이면 ORDER BY절에 사용 가능
...
-- 사수 == MANAGER_ID 컬럼에 작성된 사번이 같은 사원
...
-- 대리 직급의 직원들 중에서 과장 직급의 최소 급여보다 많이 받는 직원의 사번, 이름, 직급, 급여를 조회
...
-- 3-1) MIN을 이용하여 단일행 서브쿼리를 만듦.(다중행 서브쿼리는 비교연산자 사용X)
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME='대리'
...
-- 차장 직급의 급여의 가장 큰 값보다 많이 받는 과장 직급의 직원의 사번, 이름, 직급, 급여를 조회하세요
...
--다중열 서브쿼리 사용
--WHERE절에 작성된 "컬럼 순서"에 맞게 서브쿼리의 조회된 컬럼과 비교하여 일치하는 행 조회			 
SELECT EMP_NAME, JOB_CODE, DEPT_CODE, HIRE_DATE
FROM EMPLOYEE 
...
SELECT EMP_NO,EMP_NAME,DEPT_CODE, MANAGER_ID 사수번호, EMP_NO 주민번호, HIRE_DATE
FROM EMPLOYEE 
...
-- 본인 직급의 평균 급여를 받고 있는 직원의 사번, 이름, 직급, 급여를 조회하세요
...
-- 부서별 입사일이 가장 빠른 사원의 사번, 이름, 부서명(NULL이면 '소속없음'),
-- 직급명, 입사일을 조회하고 입사일이 빠른 순으로 조회하세요
--    단, 퇴사한 직원은 제외하고 조회하세요
SELECT EMP_NO ,EMP_NAME ,NVL(DEPT_TITLE,'소속없음'),HIRE_DATE 
FROM EMPLOYEE MAIN
...
--1)MAIN QUERY의 1행의 DEPT_CODE를 서브쿼리에 대입
--2)SUB QUERY를 수행
--3)SUB의 결과를 이용해 MAIN의 조건절 수행 
----------------------------------------------------------------------------------
...
SELECT EMP_NAME, JOB_CODE, SALARY, 
(SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE SUB WHERE SUB.JOB_CODE=MAIN.JOB_CODE) 평균
FROM EMPLOYEE MAIN
...
-- 8. RANK() OVER(정렬순서) / DENSE_RANK() OVER

-- RANK() OVER(정렬순서) : 동일한 순위 이후의 등수를 동일한 인원 수 만큼 건너뛰고 순위 계산
...
-- DENSE_RANK() OVER(정렬순서) : 동일한 순위 이후의 등수를 이후의 순위로 계산
